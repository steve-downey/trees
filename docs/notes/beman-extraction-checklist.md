# Checklist: Beman Reference Layout

## Purpose

This checklist turns the Beman integration plan into a concrete reference
layout.

It is intentionally narrower than the broader planning notes.
Its job is to answer:

- what the full reference implementation should expose
- what public names should be used at extraction time
- what belongs under the top-level public include tree
- what belongs under `detail/`
- which tests belong with each component
- which examples belong with each component
- what should remain in `trees`
...
- confirm the public headers under `include/beman/structure/`
...
- confirm that large structural helper code moves under
  `include/beman/structure/detail/`

Reference public tree:
...
- no component is considered complete until its tests move with it
...
- support machinery no longer appears as a top-level front-door header unless
  it is intentionally public support

## Tree and Container Surface
...
- move `src/smd/tree/binary_tree.hpp` to
  `include/beman/structure/binary_tree.hpp`
- move `src/smd/tree/fringe_tree.hpp` to
  `include/beman/structure/fringe_tree.hpp`
- move `src/smd/tree/finger_tree_wrappers.hpp` to
...
## Recursive Algorithm Source Material

- treat `src/smd/tree/fixpoint_tree.hpp` as source material for
  `include/beman/structure/recursive_fold.hpp`
- treat `src/smd/tree/fixpoint_tree.hpp` as source material for
  `include/beman/structure/recursive_build.hpp`
- keep the evidence path explicit: the reference layout exposes recursive
  algorithms, not a front-door `fixpoint_tree.hpp`
...
- no `detail/` header is presented as a front-door user entry point
...
- create top-level examples for the public headers
...
- every promoted public header must have at least one corresponding test target
  in `tests/`
...
- the reference layout should carry tests and examples with the component, not
  as a cleanup pass later
...
tests/
  beman/
    structure/
      ...
      functor.test.cpp
      monad.test.cpp
      dual_monoid.test.cpp
      finger_tree.test.cpp
      random_access.test.cpp
      rope.test.cpp
      priority_queue.test.cpp
      interval_index.test.cpp
      binary_tree.test.cpp
      fringe_tree.test.cpp
      finger_tree_wrappers.test.cpp
      detail/
...
examples/
  ...
  transpose_example.cpp
  fold_example.cpp
  apply_example.cpp
  recursive_fold_example.cpp
```
...
## Completion Criteria for the Reference Layout

- the Beman repo builds with the Copier-capable template baseline
- the include tree exists under `include/beman/structure/`
...
- the public API no longer exposes adapter-split headers unless intentionally
  preserved
- the front-door public API uses verb-first names where the paper plan now
  depends on them, especially `traverse`, `transpose`, `fold`, and `apply`
- large support code lives under `detail/` where appropriate
- each promoted component has tests in the Beman repo
- each top-level public surface has at least one example or equivalent usage
