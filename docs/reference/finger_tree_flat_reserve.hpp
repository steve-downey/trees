...
{
  return std::visit(
    overload{[](const Node2<T>& node) { return std::vector<T>{node.a, node.b}; },
             [](const Node3<T>& node) {
               return std::vector<T>{node.a, node.b, node.c};
             }},
    n);
}
...
class FingerTree {
 ...
 public:
  ...
  auto append(const FingerTree& right) const -> FingerTree
  {
    ...
    combined.insert(combined.end(), right_values.begin(), right_values.end());
  ...
  auto split(PREDICATE&& predicate) const -> std::optional<Split>
  {
    ...
    for (std::size_t i = 0; i < values.size(); ++i) {
      ...
      if (std::invoke(predicate, prefix)) {
        std::vector<T> left_values(values.begin(), values.begin() + static_cast<std::ptrdiff_t>(i));
        std::vector<T> right_values(values.begin() + static_cast<std::ptrdiff_t>(i + 1),
                                    values.end());
  ...
  auto split_at(PREDICATE&& predicate) const -> SplitAt
  {
    ...
    for (std::size_t i = 0; i < values.size(); ++i) {
      ...
      if (std::invoke(predicate, prefix)) {
        std::vector<T> left_values(values.begin(), values.begin() + static_cast<std::ptrdiff_t>(i));
        std::vector<T> right_values(values.begin() + static_cast<std::ptrdiff_t>(i), values.end());
  ...
  auto split_at_index(std::size_t index) const -> SplitAt
  {
    ...
    std::vector<T> left_values(values.begin(), values.begin() + static_cast<std::ptrdiff_t>(clamped));
    std::vector<T> right_values(values.begin() + static_cast<std::ptrdiff_t>(clamped), values.end());
    return SplitAt{from_sequence(std::move(left_values)),
                   from_sequence(std::move(right_values))};
  }
  ...
  {
    return split_at([&value](const TAG_TYPE& prefix) { return prefix >= value; });
  }
