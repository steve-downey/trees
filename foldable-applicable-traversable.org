...
- Prefer ~invoke~ for C++ audiences; it aligns with the ~std::invoke~ model.
- ~apply_pure(f, a, b, c)~ is a teaching alias for audiences coming from FP notation like ~[| f a b c |]~.
...
- For Applicative, that path is ~invoke~; ~apply_pure~ is a teaching aid.
