...
The predicate-based core split (~split()~ / ~split_at()~) already navigates tree structure rather than flattening.
The index-based path (~split_at_index()~, used by random-access wrapper operations) currently flattens and rebuilds (O(n)).
The point for the talk: the abstraction boundary is in the right place even before the wrapper optimization lands.
#+end_notes
...
- Predicate-based core split navigates tree structure; index-based wrapper paths currently flatten-and-rebuild (O(n)).
- The API is designed so wrapper operations can be optimized later without changing call sites.
...
The core predicate split is already structural; wrapper index paths are the remaining optimization target.
#+end_notes
