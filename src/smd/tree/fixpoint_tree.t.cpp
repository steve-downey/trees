...
TEST_CASE("FixpointTree - EvalConst")
{
    CHECK(eval(const_expr(42.0)) == 42.0);
}
...
TEST_CASE("FixpointTree - FmapExprConst")
{
    ...
    auto mapped = fmap_expr([](int x) { return std::to_string(x); }, const_layer);
...
TEST_CASE("FixpointTree - EvalAlgebraDirect")
{
    ...
    ExprF<double> layer = ExprAdd<double>{make_box<double>(10.0),
                                          make_box<double>(20.0)};
