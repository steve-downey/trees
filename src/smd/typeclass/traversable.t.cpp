...
TEST_CASE("TraversableTypeclassTest - TransposeAndTransposeWith") {
    ...
    auto transposed = traversable.transpose(identity);
    REQUIRE(transposed.has_value());
    CHECK(transposed->value == 1);
    // e7b4a1f9-3c8d-4e2a-b5f7-1d9c3e5a7b28 end

    auto transposed_with = traversable.transpose_with(traversable, identity);
    REQUIRE(transposed_with.has_value());
    CHECK(transposed_with->value == 1);
...
TEST_CASE("TraversableTypeclassTest - TransposeMatchesTraverseIdentity") {
    using IdentityOpt = smd::typeclass::test::Identity<std::optional<int>>;
    auto identity = IdentityOpt{std::optional<int>{5}};
    const auto &traversable = smd::traversable_typeclass<IdentityOpt>;

    auto via_transpose = traversable.transpose(identity);
    auto via_traverse_identity = smd::traverse(
        [](auto &&x) { return std::forward<decltype(x)>(x); }, identity);

    CHECK(via_transpose == via_traverse_identity);
}
