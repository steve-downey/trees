...
We argue once about typeclass operation names (~fold_map~, ~traverse~, ~invoke~) —
...
~push_back~, ~pop_min~, ~insert~, ~to_string~.
...
- in ~01:11~
- out ~01:15~
- cut: do 1 interpretation in detail
...
applicative typeclass object, synthesized ~apply~ from ~bind + pure~, law tests.
The Monad instance for ~optional~ is exactly ~and_then~ from C++23.
#+end_notes
...
- Q&A ~01:22~
- stop ~01:30~
- no questions: revisit payoff slides
#+end_notes
...
# bbg-footer-text: "© 2026 Bloomberg Finance L.P. All rights reserved."
