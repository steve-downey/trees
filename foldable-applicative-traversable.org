...
- Two hooks: ~pure~ (lift) and ~apply~ (apply contextual function). ~invoke~ is the C++ user API.
...
#+begin_notes
McBride's "applicative style" paper is the primary reference [cite:@mcbride2008applicative].
...
#+begin_notes
This helper (~to_vector_of_ziplists~) is hand-coded to illustrate the inverse transpose concept.
...
#+begin_notes
If the measure is a monoid, split/search become compositional [cite:@hinze2006fingertrees].
#+end_notes
...
O(log(min(n,m))) concatenation, and O(log n) split/search [cite:@hinze2006fingertrees].
The predicate-based core split (~split()~ / ~split_at()~) navigates tree structure in O(log n).
~split_at_index()~ uses a structural fast path when the measure is a count (UnitMeasure with size_t tag);
...
The ~using Impl::traverse;~ declarations in every TraversableMap class implement this selection.
A sequence-primitive Traversable would shadow ~sequence~ instead, and the base derives ~traverse~.
#+end_notes
